...
public final class Pose implements FuturePose {
    ...
    /**
     * Coordinate system for human / TeamCode field numbers (arrays, Limelight botpose, BrainSTEM push).
     * Internals (paths, follower) always use {@link PedroCoordinates}.
     * FTC apps should set this via {@code PoseConverter.useFTCCoordinates()} (done in FollowerBuilder).
     */
    private static CoordinateSystem fieldCoordinateSystem = PedroCoordinates.INSTANCE;

    public static void setFieldCoordinateSystem(CoordinateSystem coordinateSystem) {
        fieldCoordinateSystem = coordinateSystem == null ? PedroCoordinates.INSTANCE : coordinateSystem;
    }

    public static CoordinateSystem getFieldCoordinateSystem() {
        return fieldCoordinateSystem;
    }

    /**
     * Field / TeamCode pose → Pedro pose used everywhere internally.
     */
    public static Pose fromField(double x, double y, double headingRadians) {
        return new Pose(x, y, headingRadians, fieldCoordinateSystem)
                .getAsCoordinateSystem(PedroCoordinates.INSTANCE);
    }

    public static Pose fromFieldDegrees(double x, double y, double headingDegrees) {
        return fromField(x, y, Math.toRadians(headingDegrees));
    }
